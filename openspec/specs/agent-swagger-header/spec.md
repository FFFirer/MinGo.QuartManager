# Agent Swagger Header

## Purpose

为 Agent 相关 API 端点提供统一的 Swagger UI Header 参数标注机制。通过 `[SwaggerHeader]` 自定义特性 + NSwag `IOperationProcessor`，Controller 和 Minimal API 端点可以使用同一套声明式方式使 Header 参数在 OpenAPI 文档中可见。
