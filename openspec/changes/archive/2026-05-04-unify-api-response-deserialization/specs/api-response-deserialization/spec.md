...
#### Scenario: ApiResponseException 继承 AgentException
- **WHEN** `ApiResponseException` 被定义
- **THEN** 它继承自 `AgentException`
- **AND** 包含 `ErrorCode` 属性（`string?`）
- **AND** 构造函数接受 `message` 和可选的 `errorCode` 参数

#### Scenario: ApiResponseException 在 Direction B 中被 catch
- **WHEN** `AgentProxyService.HandleResponse<T>` 捕获到 `ApiResponseException`
- **THEN** 由于继承 `AgentException`，现有的 `catch (AgentException)` 块能正常捕获

### Requirement: AgentProxyService.HandleResponse 使用新的解包逻辑
...
- **THEN** `HandleResponse` 抛出 `ApiResponseException`（可被上层 catch 为 `AgentException`）

#### Scenario: 非 200 HTTP 状态码
- **WHEN** Agent 返回 HTTP 4xx/5xx
- **THEN** `HandleResponse` 继续使用原有逻辑，抛 `AgentException`（不改变）

### Requirement: Agent 端 HTTP 调用使用新的解包逻辑
